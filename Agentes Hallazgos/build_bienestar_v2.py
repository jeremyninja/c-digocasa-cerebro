#!/usr/bin/env python3
"""
build_bienestar_v2.py
=====================
Construye el deck bienestar-deck-flat-v2.pptx desde cero con python-pptx.
23 slides según el set editorial cerrado bienestar-hallazgos-editados.md.

Reglas aplicadas (mayo 2026 — versión corregida):
- Fondo negro plano #000000 (sin masterslide decorativo)
- Headlines en MAYÚSCULAS auto-size según tabla de caracteres
- Stats grandes redondeados a entero (decimal solo si 1 dígito antes del punto)
- Consumer Voice con TODOS los verbatims en UN SOLO slide como cards #2E2E2E
...
# ── rutas ──────────────────────────────────────────────────────────────────────
OUTPUT_PATH = "/Users/jeremyrodriguez/Documents/Cerebro/Código Casa/Agentes Hallazgos/bienestar-deck-flat-v2.pptx"
...
def add_headline(slide, text_plain, text_italic=""):
    ...
    Centrado horizontalmente, posición arriba (1" del top).
...
def add_vertical_separator(slide, x_inches, y_top_inches, height_inches):
    ...
    line.line.fill.background()  # sin borde
...
def add_card(slide, quote_text, attribution, x_left_inches, y_top_inches,
    ...
    Fondo #2E2E2E sólido, border-radius simulado, comillas españolas.
...
def build_hallazgo_slide(prs, headline_plain, headline_italic,
    ...
    desc_runs: lista de runs (make_run)
    """
    ...
    if n >= 2:
        # separador entre stat 1 y stat 2
        x_sep1 = (xs[0] + xs[1]) / 2
        add_vertical_separator(slide, x_sep1, y_stat - 0.1, 1.6)
    if n == 3:
        # separador entre stat 2 y stat 3
        x_sep2 = (xs[1] + xs[2]) / 2
...
def build_consumer_voice_slide(prs, verbatims):
    """
    Construye un slide Consumer Voice con TODOS los verbatims del hallazgo.
    verbatims: lista de dicts {quote, attribution}
    Todos van en UN SOLO slide como cards apiladas verticalmente.
    """
    slide = blank_slide(prs)
    ...
    # Calcular geometría según número de cards
    margin_side = 1.2
    card_width  = 13.33 - (2 * margin_side)  # ~10.93"
    total_height_available = 7.5 - 1.3 - 0.3  # desde y=1.3 hasta y=7.2
    gap_between = 0.25

    if n_cards == 1:
        card_height = min(total_height_available, 4.8)
        y_start     = 1.3 + (total_height_available - card_height) / 2
    elif n_cards == 2:
        card_height = (total_height_available - gap_between) / 2
        card_height = min(card_height, 2.8)
        total_cards_h = card_height * 2 + gap_between
        y_start = 1.3 + (total_height_available - total_cards_h) / 2
    else:  # 3 cards (no ocurre en este set pero por si acaso)
        card_height = (total_height_available - 2 * gap_between) / 3
        card_height = min(card_height, 2.0)
        total_cards_h = card_height * 3 + 2 * gap_between
        y_start = 1.3 + (total_height_available - total_cards_h) / 2

    for i, v in enumerate(verbatims):
        y_card = y_start + i * (card_height + gap_between)
        add_card(
            slide,
            quote_text=v['quote'],
...
def build_deck():
    ...
    # ── H01 — Slide 1: Hallazgo cuanti (2 stats) ──────────────────────────────
    ...
    # ── H01 — Slide 2: Consumer Voice (1 verbatim) ────────────────────────────
    ...
    # ── H02 — Slide 3: Hallazgo cuanti (1 stat) ───────────────────────────────
    ...
    # ── H02 — Slide 4: Consumer Voice (1 verbatim) ────────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
    ...
    # ── H03 — Slide 5: Hallazgo cuanti (2 stats) ──────────────────────────────
    ...
    # ── H03 — Slide 6: Consumer Voice (2 verbatims) ───────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
            ...
            {
                'quote': 'Me refugio mucho en Dios, en la oración. Tener una conexión con Dios para mí es importante, dejarle las cosas a él, que él se encargue (...) porque hay días que tú dices "yo no puedo más" y Dios se encarga. Y escuchar podcast de salud mental, cualquier libro que pueda ayudarte personalmente a alcanzar cualquier cosa.',
                'attribution': 'Familia biparental con hijos adultos'
            }
        ]
    )

    # ── H04 — Slide 7: Hallazgo solo-cuanti (2 stats, sin verbatim) ───────────
    ...
    # ── H05 — Slide 8: Hallazgo cuanti (2 stats) ──────────────────────────────
    # Headline 163 chars → 30pt
    # Stats: 43%, y descripción de breakdown tipologías
    build_hallazgo_slide(
        prs,
    ...
    # ── H05 — Slide 9: Consumer Voice (1 verbatim) ────────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
    ...
    # ── H06 — Slide 10: Hallazgo cuanti (2 stats) ─────────────────────────────
    ...
    # ── H06 — Slide 11: Consumer Voice (2 verbatims) ──────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
            ...
            {
                'quote': 'Yo tenía mucho tiempo yendo al gimnasio y decidí tomar una pausa por temas laborales (...) me siento menos cargado, increíblemente, porque el gimnasio ayudó a liberar muchas cosas.',
                'attribution': 'Familia sin hijos con mascota'
            }
        ]
    )

    # ── H07 — Slide 12: Hallazgo cuanti (3 stats) ─────────────────────────────
    ...
    # ── H07 — Slide 13: Consumer Voice (1 verbatim) ───────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
    ...
    # ── H08 — Slide 14: Hallazgo cuanti (3 stats) ─────────────────────────────
    ...
    # ── H08 — Slide 15: Consumer Voice (2 verbatims) ──────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
                'quote': 'Mis hijos, los amo y los quiero, pero también son iguales (...) las escuelas, los colegios son expertos en ponerte una misma reunión de dos grados distintos a la misma hora (...) ese tema de que dependa de mí 24-7 lo que se vaya a hacer te da un poco de estrés.',
    ...
    # ── H09 — Slide 16: Hallazgo cuanti (2 stats) ─────────────────────────────
    ...
    # ── H09 — Slide 17: Consumer Voice (2 verbatims) ──────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
                'quote': '(...) en nuestro caso, las que somos madres solteras, con tantas cosas encima y que no tengamos ese espacio o ese momento (...) lamentablemente en alguien tú te vas a descargar, y quién tú tienes al lado para descargarte — lamentablemente son los hijos.',
                'attribution': 'Familia monoparental'
            },
            {
    ...
    # ── H10 — Slide 18: Card cualitativa (solo-cuali, 2 verbatims) ────────────
    ...
    # ── H11 — Slide 19: Hallazgo cuanti (3 stats) ─────────────────────────────
    ...
    # ── H11 — Slide 20: Consumer Voice (1 verbatim) ───────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
    ...
    # ── H12 — Slide 21: Card cualitativa (solo-cuali, 1 verbatim) ────────────
    # Headline 189 chars → 30pt
    build_cuali_slide(
        prs,
    ...
    # ── H13 — Slide 22: Hallazgo cuanti (1 stat) ──────────────────────────────
    # Headline 170 chars → 30pt
    # Stat: 49% (stat grande redondeado) — pero mostramos la brecha en la descripción
    build_hallazgo_slide(
        prs,
    ...
    # ── H13 — Slide 23: Consumer Voice (2 verbatims) ──────────────────────────
    build_consumer_voice_slide(
        prs,
        verbatims=[
            {
            ...
            {
                'quote': 'Es sano, eso, no dedicarse un tiempo solo. Por ejemplo, si yo quisiera, qué sé yo, sentarme a mirar el parque y tomarme una batida yo sola así... cómo no me hace falta...',
                'attribution': 'Familia biparental con hijos pequeños'
            }
        ]
    )
